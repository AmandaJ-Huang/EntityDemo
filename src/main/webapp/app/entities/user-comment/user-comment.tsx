  ...
  return (
    <div>
      <h2 id="user-comment-heading" data-cy="UserCommentHeading">
        <Translate contentKey="entityDemoApp.userComment.home.title">User Comments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="entityDemoApp.userComment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="entityDemoApp.userComment.home.createLabel">Create new User Comment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {userCommentList && userCommentList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="entityDemoApp.userComment.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {userCommentList.map((userComment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userComment.appUser ? <Link to={`app-user/${userComment.appUser.id}`}>{userComment.appUser.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="entityDemoApp.userComment.home.notFound">No User Comments found</Translate>
            </div>
          )
        )}
      </div>
